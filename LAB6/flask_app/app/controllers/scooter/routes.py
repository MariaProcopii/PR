...
from app.controllers.scooter import bp
...
def index():
    ...
    return (
        jsonify(
            {
                "electro_scooters_list": [
                    {"name": e.name, "battery_level": e.battery_level}
                    for e in electro_scooters
                ]
            }
        ),
        200,
...
def create_scooter():
    ...
    electro_scooter = ElectroScooter(data.get("name"), data.get("battery_level"))
